<template>
  <div>
    ...
    <form
      ...
      <template v-if="loading.research">
      ...
      <table class="data-block text-center">
        ...
        <template v-if="!editMode">
          <tr v-for="activity of research" :key="activity.id">
        ...
        <template v-else>
          ...
          <tr class="edit">
            <td colspan="3" class="cursor-pointer" @click="addRow">
              +
            </td>
          </tr>
        </template>
...
export default {
  ...
  methods: {
    ...
    save() {
      this.$store.dispatch('saveResearch', this.rows)
      this.toggleEdit()
    },
  },

  computed: {
    ...mapState(['research', 'loading']),
    total_semanal() {
