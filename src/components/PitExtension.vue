  ...
  <div>
    ...
    <form
      ...
      <table class="data-block text-center">
        ...
        <template v-if="!editMode">
          <tr v-for="activity of extension" :key="activity.id">
            <td>{{ activity.atividade }}</td>
            <td>{{ activity.participacao }}</td>
        ...
        <template v-else>
          <tr v-for="row of rows" :key="row.id" class="edit">
            ...
            <td>
              <input v-model="row.participacao" type="text">
            </td>
            <td>
...
export default {
  ...
  methods: {
    ...
    save() {
      this.$store.dispatch('saveExtension', this.rows)
      this.toggleEdit()
    },
  },

  computed: {
    ...mapState(['extension', 'loading']),
    total_semanal() {
